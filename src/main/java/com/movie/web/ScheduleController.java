...
import com.movie.repository.ScheduleDao;
...
*/
	...
	public String scheduleCalendar(Model model) {
		...
		//System.out.println(min + "," + max);
		model.addAttribute("min", min); //2018-12-06
		model.addAttribute("max", max);
	...
	public String scheduleAll(@RequestParam String pickDate, Model model, @RequestParam String min, @RequestParam String max) {

		//System.out.println(pickDate); 
		List<MovieVO> list = scheduleDao.getScheduleMovie(pickDate);
		...
		for(MovieVO vo : list) {
			...
			// 시간 포맷 설정 24시간 (HH:mm) / 12시간 (hh:mm)
			SimpleDateFormat sdf2 = new SimpleDateFormat("HH:mm", Locale.KOREA);
			String now = sdf2.format(System.currentTimeMillis());
			 ...
			 // 현재날짜 영화 시작 마지막 비교하기
			 int compare = today.compareTo(pickDate);
			 
			 // 현재시간 비교하기
			 int compareTime = now.compareTo(vo.getMv_time());
			 
			 if(compare >=0) {	
				 System.out.print("상영일이 오늘이거나 지나감. ");
				 if (compareTime >= 0) {
					 vo.setMv_isTrue("F");
						 System.out.print("시간이 지나감");
				} 
		...
		// 상영중인 모든 영화 가져오기
		List<MovieVO> All = movieDao.getMovieTitle();
		model.addAttribute("All", All);
		model.addAttribute("pickDate", pickDate);	
		
						
		return "/schedule/scheduleAll";
	}
	
	// mv_num으로..
	@RequestMapping(value="selectSeat2", method=RequestMethod.POST)
	public String selectSeat2(@ModelAttribute MovieVO movie,@ModelAttribute BookVO book, @RequestParam("mv_title") String mv_title, Model model) {
		
		// 1.mv_num 가져오기
		MovieVO movieVO = scheduleDao.getMovieMvnum(mv_title, movie.getMv_time(), movie.getTt_num());
