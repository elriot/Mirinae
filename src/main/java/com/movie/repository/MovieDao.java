...
public class MovieDao {
	...
	public MovieVO getMovieByMvTitle(String mv_title) {
		MovieVO movieVO = jdbcTemplate.queryForObject("select mv_title, mv_rating, mv_detail, mv_releaseDate, mv_startDate, mv_endDate, mv_postImage, mv_trailer from movie where mv_title=?",
				new BeanPropertyRowMapper<MovieVO>(MovieVO.class), mv_title);
		return movieVO;
	}	
	...
	public List<MovieVO>getMoviesForMainPage() {
		List<MovieVO> list = jdbcTemplate.query("select DISTINCT mv_num, mv_title, mv_rating, mv_detail, mv_releasedate, mv_postimage, mv_trailer from movie where mv_isTrue='T'"
				, new BeanPropertyRowMapper<MovieVO>(MovieVO.class));
		return list;
	}
